...
if __name__ == '__main__':
    group_index = str(sys.argv[1])
    solution_dir_path = './random_solutions/'
    if not os.path.exists(solution_dir_path):
        os.mkdir(solution_dir_path)
    input_dir_path = './random_data/data_' + group_index
    config = configparser.ConfigParser()  # 类实例化
    # 定义参数文件路径
    path = input_dir_path + '/config.ini'
    config.read(path)
    # 导入参数配置
    ...
    # qos_constraint = 400
    # 客户
    with open(input_dir_path + '/demand.csv', 'rb') as f:
    ...
    with open(input_dir_path + '/site_bandwidth.csv', 'rb') as s:
    ...
    with open(input_dir_path + '/qos.csv', 'rb') as q:
    ...
    solution = open(solution_dir_path + 'solution_' + group_index + '.txt', 'w')
