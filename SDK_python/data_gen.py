...
def read_input(s_num_input, c_client_input, time_len_input, pressure_input):
    global server_num, client_num, time_len, pressure
    # server_num = ask('input server number (default 100):', 100)
    # client_num = ask('input client number (default 10):', 10)
    # time_len = ask('input time length (default 100):', 100)
    # inp = input('input pressure 0.01-0.99 (default 0.6):')
    server_num = s_num_input
    client_num = c_client_input
    time_len = time_len_input
    pressure = float(pressure_input)
    # print('Server num: {}, client num: {}, time len: {}, pressuse: {}'.format(server_num, client_num, time_len, pressure))
...
if __name__ == '__main__':
    ...
    server_num = np.random.randint(50, 151)
    client_num = np.random.randint(5, 50)
    time_len = np.random.randint(50, 151)
    pressure = float(sys.argv[2]) / 100.0
    read_input(server_num, client_num, time_len, pressure)
    record = np.zeros((time_len, server_num, client_num), dtype=np.int32)
    distribute_server()
    if len(sys.argv) == 1:
        output('pressure_data')
    else:
        try: os.mkdir('random_data/' + sys.argv[1])
        except: pass
        output('random_data/' + sys.argv[1])
